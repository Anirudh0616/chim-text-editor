...
#define chim_version "0.1.5" // new update
...
char *C_HL_keyword[] = { "switch", "if", "while", "for", "break", "continue", "return", "else", "struct", "union", "typedef", "static", "enum", "class", "case",
...
void editorUpdateRowSyntax (erow *row){
    ...
    if(changed && row->idx + 1 < E.numrows){
        editorUpdateRowSyntax(&E.row[row->idx + 1]);
    }

    if (E.search_query && E.search_query[0] != '\0') {
        int query_len = strlen(E.search_query);
        char *match = row->render;
        while ((match = strstr(match, E.search_query)) != NULL) {
            memset(&row->hl[match - row->render], HL_MATCH, query_len);
            match += query_len;
        }
    }
}
        ...
        while(s->filematch[i]){
            ...
            if ((is_ext && ext && !strcmp(ext, s->filematch[i])) || (!is_ext && strstr(E.filename, s->filematch[i]))){
                ...
                for(filerow = 0; filerow<E.numrows; filerow++){
                    editorUpdateRowSyntax(&E.row[filerow]);
                }
                return ;
            }
            i++;
...
void editorDrawStatusBar(struct abuf* ab)
{
    ...
    char *ft = E.syntax ? E.syntax->filetype : " ";
    int ftlen = strlen(ft);
	...
	// Draw bar
	int len = modeShow;
	while (len < E.screencols - filelen - lslen - 4 - dirtylen - ftlen - 1) {
		abAppend(ab, " ", 1);
		len++;
	}
    abAppend(ab, ft, ftlen);
    abAppend(ab, " ", 1);
